...
/// Metrics registry for the derived materialize job.
///
/// Tracks ingestion, write, freshness, and dump metrics per dataset/job.
/// Unlike the raw variant, this does not include a `latest_block` gauge
/// because derived materialization never sets a latest block number.
#[derive(Debug, Clone)]
