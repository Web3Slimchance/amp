...
use crate::meta::{PARQUET_METADATA_KEY, ParquetMeta};

/// Reads a Parquet file from the store and serializes its footer metadata to bytes.
#[tracing::instrument(skip(object_meta, store), err)]
...
/// Extracts Amp-specific metadata, the resolved filename, and footer bytes from a Parquet file.
#[tracing::instrument(skip(object_meta, store), err)]
...
/// Reads Parquet metadata (including column and offset indices) from the file footer.
