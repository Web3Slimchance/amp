...
fn schema() -> Schema {
    Schema::new(vec![
        ...
        Field::new("general_gas_limit", DataType::UInt64, false),
        Field::new("shared_gas_limit", DataType::UInt64, false),
        Field::new("timestamp_millis_part", DataType::UInt64, false),
    ])
}
...
pub struct BlockRowsBuilder {
    ...
    // Tempo-specific
    general_gas_limit: UInt64Builder,
    shared_gas_limit: UInt64Builder,
    timestamp_millis_part: UInt64Builder,
}
...
impl BlockRowsBuilder {
    pub fn with_capacity_for(header: &Block) -> Self {
        Self {
            ...
            general_gas_limit: UInt64Builder::with_capacity(1),
            shared_gas_limit: UInt64Builder::with_capacity(1),
            timestamp_millis_part: UInt64Builder::with_capacity(1),
        }
    }
    ...
    pub fn append(&mut self, row: &Block) {
        ...
        self.general_gas_limit.append_value(*general_gas_limit);
        self.shared_gas_limit.append_value(*shared_gas_limit);
        self.timestamp_millis_part
            .append_value(*timestamp_millis_part);
    }
    ...
    pub fn build(self, range: BlockRange) -> Result<TableRows, TableRowError> {
        let Self {
            ...
            parent_beacon_root,
            requests_hash,
            mut general_gas_limit,
            mut shared_gas_limit,
            mut timestamp_millis_part,
        } = self;
        ...
        let columns = vec![
            ...
            Arc::new(general_gas_limit.finish()),
            Arc::new(shared_gas_limit.finish()),
            Arc::new(timestamp_millis_part.finish()),
        ];
...
fn default_to_arrow() {
    ...
    // 23 inner header + 3 tempo-specific + _block_num + _ts = 28
    assert_eq!(rows.rows.num_columns(), 28);
